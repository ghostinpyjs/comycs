...
function getStat(stats, name) { return Number(stats?.find(s => s.name === name)?.value || 0); }
function parseStats(raw) {
  const s = raw?.playerstats?.stats || [];
  const kills = getStat(s, 'total_kills'), deaths = getStat(s, 'total_deaths'), wins = getStat(s, 'total_wins');
  const rounds = getStat(s, 'total_rounds_played'), hs = getStat(s, 'total_kills_headshot');
  const shots = getStat(s, 'total_shots_fired'), hits = getStat(s, 'total_shots_hit');
  const mvps = getStat(s, 'total_mvps'), damage = getStat(s, 'total_damage_done');
  const bp = getStat(s, 'total_planted_bombs'), bd = getStat(s, 'total_defused_bombs');
  const knifeK = getStat(s, 'total_kills_knife'), awpK = getStat(s, 'total_kills_awp');
...
function Err({ msg }) { return <div className="err"><span>⚠</span> {msg}</div>; }
    ...
    <div className="overlay" onClick={e => e.target===e.currentTarget&&onClose()}>
      <div className="modal">
        ...
        <div className="modal-body">
          ...
          <div className="modal-img-area">{item.icon_url?<img src={item.icon_url} alt={item.name} className="modal-img"/>:<div style={{fontSize:56,color:'var(--text3)'}}>🔫</div>}</div>
...
function StatsTab({ steamid }) {
  ...
  useEffect(()=>{ if(!steamid)return; setLoading(true);setErr(''); fetch(`/api/stats?steamid=${steamid}`).then(r=>r.json()).then(d=>{setData(d);setLoading(false);}).catch(()=>{setErr('Erro ao carregar stats');setLoading(false);}); },[steamid]);
...
function InventoryTab({ steamid }) {
  const [data,setData]=useState(null),[loading,setLoading]=useState(false),[err,setErr]=useState(''),[filter,setFilter]=useState('Todos'),[selected,setSelected]=useState(null);
  useEffect(()=>{ if(!steamid)return; setLoading(true);setErr(''); fetch(`/api/inventory?steamid=${steamid}`).then(r=>r.json()).then(d=>{if(d.error)setErr(d.error);else setData(d);setLoading(false);}).catch(()=>{setErr('Erro ao carregar inventário');setLoading(false);}); },[steamid]);
  ...
  return (
    <div>
      ...
      {filtered.length===0?<Empty icon="📦" title="Nenhum item" text="Sem itens nessa categoria."/>:(
...
function FeedTab({ currentPlayer, currentSteamid }) {
  const [posts,setPosts]=useState([]),[loadingPosts,setLoadingPosts]=useState(true),[content,setContent]=useState(''),[posting,setPosting]=useState(false),[friends,setFriends]=useState([]);
  const MAX=280;
  ...
  const submitPost=async()=>{ if(!content.trim()||content.length>MAX)return; setPosting(true); const res=await fetch('/api/posts',{method:'POST',headers:{'Content-Type':'application/json'},body:JSON.stringify({steamid:currentSteamid||'',username:currentPlayer?.personaname||'Anônimo',avatar:currentPlayer?.avatarmedium||null,content:content.trim()})}); const d=await res.json(); if(d.post)setPosts(prev=>[d.post,...prev]); setContent('');setPosting(false); };
  const toggleLike=async(id)=>{ const res=await fetch('/api/posts',{method:'PATCH',headers:{'Content-Type':'application/json'},body:JSON.stringify({id,steamid:currentSteamid||'anon'})}); const d=await res.json(); if(d.post)setPosts(prev=>prev.map(p=>p.id===id?d.post:p)); };
      ...
      <div>
        <div className="feed-compose">
          <div className="compose-header">
            {currentPlayer?.avatarmedium?<img src={currentPlayer.avatarmedium} alt="" className="compose-avatar" style={{borderRadius:'var(--radius-sm)'}}/>:<div className="compose-avatar">👤</div>}
            <div><div className="compose-name">{currentPlayer?.personaname||'Visitante'}</div><div className="compose-sub">{currentPlayer?'Conectado via Steam':'Carregue seu perfil para postar'}</div></div>
          </div>
          ...
          <div className="compose-footer"><span className={`compose-chars ${content.length>MAX?'warn':''}`}>{content.length}/{MAX}</span><button className="compose-btn" onClick={submitPost} disabled={posting||!content.trim()||content.length>MAX}>{posting?'Postando...':'Publicar'}</button></div>
        </div>
        {loadingPosts?<Loading text="CARREGANDO FEED..."/>:posts.length===0?<Empty icon="💬" title="Feed vazio" text="Seja o primeiro a postar!"/>:posts.map((post,i)=>(
          <div key={post.id} className="post-card" style={{animationDelay:`${i*0.04}s`}}>
            <div className="post-header">{post.avatar?<img src={post.avatar} alt="" className="post-avatar"/>:<div className="post-avatar" style={{display:'flex',alignItems:'center',justifyContent:'center',background:'var(--bg4)',fontSize:18}}>👤</div>}<div><div className="post-user">{post.username}</div><div className="post-time">{timeAgo(post.createdAt)}</div></div></div>
            <div className="post-content">{post.content}</div>
            <div className="post-footer"><button className={`like-btn ${post.likedBy?.includes(sid)?'liked':''}`} onClick={()=>toggleLike(post.id)}>{post.likedBy?.includes(sid)?'❤️':'🤍'} {post.likes}</button><button className="share-btn" onClick={()=>navigator.clipboard?.writeText(window.location.href)}>🔗 Compartilhar</button></div>
          </div>
        ))}
      </div>
      <div className="sidebar">
        {currentPlayer&&(
          <div className="sidebar-card">
            ...
            {friends.length===0?<div style={{padding:'16px 18px',fontSize:13,color:'var(--text3)'}}>Lista de amigos privada ou vazia.</div>:friends.slice(0,10).map(f=>{const st=statusInfo(f.status);return(<a key={f.steamid} href={f.profileurl} target="_blank" rel="noopener noreferrer" className="sidebar-item"><img src={f.avatar} alt="" className="sidebar-item-av"/><div style={{flex:1,minWidth:0}}><div className="sidebar-item-name">{f.username}</div><div className="sidebar-item-sub">{f.gameextrainfo||st.label}</div></div><div className="sidebar-item-status"><div className={st.dot}/></div></a>);})}
          </div>
        )}
        <div className="sidebar-card"><div className="sidebar-title">📌 Sobre o COMYCS</div><div style={{padding:'14px 18px',fontSize:13,color:'var(--text2)',lineHeight:1.7}}>Plataforma de stats e comunidade para jogadores de CS2. Carregue seu perfil Steam para ver suas estatísticas, inventário e interagir com outros jogadores.</div></div>
      </div>
    </div>
  );
}
...
function RankingTab({ onLoadProfile }) {
  const [ranking,setRanking]=useState([]),[loading,setLoading]=useState(true),[sort,setSort]=useState('kills');
  ...
  return (
    <div>
      ...
      {sorted.length===0?<div className="rank-empty"><div className="rank-empty-icon">🏆</div><div className="rank-empty-txt">O ranking será preenchido conforme os jogadores carregarem seus perfis.</div></div>:(
          ...
          <table className="ranking-table">
            ...
            <tbody>{sorted.map((p,i)=>(
              <tr key={p.steamid}>
                ...
                <td><div className="rank-player">{p.avatar?<img src={p.avatar} alt="" className="rank-av"/>:<div className="rank-av" style={{display:'flex',alignItems:'center',justifyContent:'center',background:'var(--bg4)',fontSize:14}}>👤</div>}<button onClick={()=>onLoadProfile(p.steamid)} className="rank-name" style={{background:'none',border:'none',cursor:'pointer',textAlign:'left'}}>{p.username}</button></div></td>
...
function ProfilesTab({ onLoadProfile }) {
  const [query,setQuery]=useState(''),[result,setResult]=useState(null),[loading,setLoading]=useState(false),[err,setErr]=useState('');
  const search=async()=>{ if(!query.trim())return; setLoading(true);setErr('');setResult(null); try{ const res=await fetch(`/api/resolve?vanity=${encodeURIComponent(query.trim())}`); const d=await res.json(); if(d.error){setErr(d.error);setLoading(false);return;} const pRes=await fetch(`/api/player?steamid=${d.steamid}`); const pData=await pRes.json(); const p=pData?.response?.players?.[0]; if(!p){setErr('Jogador não encontrado');setLoading(false);return;} setResult({...p}); }catch{setErr('Erro na busca');} setLoading(false); };
  const st=result?statusInfo(result.personastate):null;
  return (
    <div className="profile-search-wrap">
      <div style={{marginBottom:24}}><div style={{fontFamily:'var(--font-display)',fontSize:22,fontWeight:800,marginBottom:6}}>Buscar Perfis</div><div style={{fontSize:14,color:'var(--text2)'}}>Digite um nome de usuário Steam ou SteamID64.</div></div>
        ...
        <div className="found-profile-card">
          ...
          <div className="found-info">
            ...
            <div style={{marginTop:8,display:'flex',alignItems:'center',gap:6}}>
              <div className={`profile-status ${st?.cls}`} style={{display:'inline-flex',alignItems:'center',gap:5,padding:'3px 10px',borderRadius:20,fontSize:12,fontWeight:600}}><span className="s-dot" style={{width:6,height:6,borderRadius:'50%',background:'currentColor'}}/>{st?.label}</div>
      ...
      {!result&&!loading&&!err&&<div style={{marginTop:32,padding:'24px',background:'var(--bg3)',border:'1px solid var(--border)',borderRadius:'var(--radius)',fontSize:14,color:'var(--text3)',lineHeight:1.8}}><div style={{fontWeight:600,color:'var(--text2)',marginBottom:8}}>💡 Dica</div>Busque por URL personalizada (ex: <b style={{color:'var(--text)'}}>gaben</b>) ou SteamID64.</div>}
    </div>
  );
}

// ─── SteamIcon reutilizável ───────────────────────────────────────────────────
...
export default function Home({ steamUser, onLogout }) {
  ...
  // Auto-carrega perfil quando usuário faz login via Steam
  useEffect(() => {
  ...
  const loadProfile = useCallback(async (query) => {
    ...
    try {
      ...
      const [playerRes, playtimeRes] = await Promise.allSettled([
        ...
        fetch(`/api/stats?steamid=${sid}`).then(r => r.json()).then(sd => {
          if (sd?.stats) {
            const s = parseStats(sd.stats);
      ...
      if (playerRes.status === 'fulfilled') { const p = playerRes.value?.response?.players?.[0]; if (p) setPlayer(p); else setErr('Jogador não encontrado'); }
    ...
    setLoading(false); setPage('meu-perfil');
  }, [input]);
  ...
  return (
    <>
      ...
      <main className="main">
          ...
          <div className="hero">
            ...
            {!steamUser && (
              <div className="hero-login-area">
                <div className="hero-divider"><span>ou entre com sua conta Steam</span></div>
                <a href="/api/auth/steam" className="hero-steam-btn">
                  <SteamIcon size={18}/> Entrar com Steam — carrega seu perfil automaticamente
                </a>
              </div>
            )}
        ...
        {page==='feed'&&<div className="page-tabs-wrap" style={{animationName:'fadeIn',animationDuration:'.3s'}}><div style={{fontFamily:'var(--font-display)',fontSize:24,fontWeight:800,marginBottom:20}}>💬 Feed da Comunidade</div><FeedTab currentPlayer={player} currentSteamid={steamid}/></div>}
        {page==='ranking'&&<div className="page-tabs-wrap"><div style={{fontFamily:'var(--font-display)',fontSize:24,fontWeight:800,marginBottom:20}}>🏆 Ranking Global</div><RankingTab onLoadProfile={handleLoadFromRanking}/></div>}
        {page==='perfis'&&<div className="page-tabs-wrap"><ProfilesTab onLoadProfile={handleLoadFromRanking}/></div>}
      </main>

      <footer className="footer"><b>COMYCS</b> — CS2 Stats & Comunidade · Dados via Steam Web API · Não afiliado à Valve Corporation</footer>
    </>
  );
}
