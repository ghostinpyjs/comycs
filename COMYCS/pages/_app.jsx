...
export default function App({ Component, pageProps }) {
  ...
  // Captura ?steamid=...&login=success após o callback OpenID
  useEffect(() => {
    ...
    if (login === 'success' && steamid) {
      ...
      fetch(`/api/player?steamid=${steamid}`)
        ...
        .then(data => {
          ...
          if (p) {
            const user = { steamid: p.steamid, username: p.personaname, avatar: p.avatarmedium || p.avatar, profileUrl: p.profileurl };
            setSteamUser(user);
  ...
  useEffect(() => {
    ...
    if (saved) { try { setSteamUser(JSON.parse(saved)); } catch { sessionStorage.removeItem('steamUser'); } }
  }, []);
  ...
  return (
    <>
      <Component {...pageProps} steamUser={steamUser} setSteamUser={setSteamUser} onLogout={handleLogout}/>
      <Analytics/>
    </>
  );
}
