...
export default async function handler(req, res) {
  ...
  try {
    ...
    for (const asset of assets.slice(0, 50)) {
      ...
      items.push({ name: desc.market_hash_name, icon_url: desc.icon_url, tags: desc.tags ?? [], price_usd: priceUSD, price_brl: (priceUSD * 5).toFixed(2) });
    }

    const totalUSD = items.reduce((s, i) => s + i.price_usd, 0).toFixed(2);
    const totalBRL = (parseFloat(totalUSD) * 5).toFixed(2);
